...
* https://songshuhui.net/archives/76501

# 统计基本概念

* 统计是一门艺术(人为的)
* 数理统计是一门应用性很强的学科，它是研究怎样以有效的方式收集、整理和分析带有随机性的数据，以便对所考察的问题推断和预测，为决策和行动提供依据和建议
* 大量的随机现象必然呈现它的规律性，理论上，只要对随机现象足够多次观察，随机现象规律性一定被能清楚的呈现出来
* 客观情况往往只允许少量观察试验，我们只能获得局部观察资料，数理统计的任务就是用有限的资料，尽可能的作出可靠而精确的结论
* 描述性统计(传统统计：比例，中位数，折线图)
* 推断性统计(利用观察到的数据对总体作出某种推断)
    * 抽样(一门课程)
    * 实验(一门课程)
    * 统计方法
    * 估计(estimate) => [点估计, 区间估计] (概率模型假定下，根据数据对概率分布的未知参数估计)
    * 检验(test) => [参数检验, 非参数检验] (根据数据对概率分布中的未知参数进行检验)
...
* 统计量：为了刻画某个特征，对样本的一种加工(本质上就是随机变量的函数)(也叫抽样分布)
    * 样本均值：X-bar = 1/n * ∑Xi
    * 样本方差：S^2 = 1/(n-1) * ∑(Xi - X-bar)^2
    * 统计量的作用在于集中有用的信息，降低数据的维度
* X-bar ~ N(μ, σ^2/n)
* (n-1)*S^2/σ^2 ~ 自由度为 n-1 的卡方分布
* X-bar 与 S^2 独立
