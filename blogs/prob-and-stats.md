...
* 切比雪夫不等式 P(|X - μ|>=kσ) <= 1/k^2
    ...
    * 若一班有36个学生，而在一次考试中，平均分是80分，标准差是10分，我们便可得出结论：少于50分（与平均相差3个标准差以上）的人，数目不多于4个（=36*1/9）
    * 切比雪夫不等式预测的准确率要远远高于马尔科夫不等式(因为切比雪夫用到了方差)
* 相关系数是衡量随机变量间的线性关系的 |ρ|<=1

## ch4 大数定律 & 中心极限定理
* 大数定律指出，当 n -> ∞ 时，样本均值 X-bar -> μ
* 大叔定律指出，当 n -> ∞ 时，X-bar 是一个变化非常小的随机变量
* 大叔定律指出，当 n 比较大时，可以用 X-bar 代替总体的真实均值：即均值估计
...
* 统计量：为了刻画某个特征，对样本的一种加工(统计量由样本的函数唯一确定 <=> Statistic = ξ(Sample))
    * 样本均值：X-bar = 1/n * ∑Xi
